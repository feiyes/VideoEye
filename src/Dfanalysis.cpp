		...
		for(i=0;i<mb_width;i++){
			...
			case DRAW_Q:{
				...
				if(prop_q_showcolor_val!=FALSE){
					...
					//等待画完一帧然后每个Create一次Brush，当然需要及时的DeleteObject
			...
			case DRAW_MV0:{
				...
				//注意:FFMPEG中运动向量是按宏块存储的，并没有直接的关系。
				//因此在此程序中需要通过计算宏块序号。
				//计算8x8块值对应的运动向量之间的对应关系？
				//-------------------------
				//|			 |			  |
				//|mv[x]	 |mv[x+1]	  |
				//-------------------------
				...
				//一个MV的个数
				//FIX:目前测试H.264的mv_stride和MPEG4、RMVB不太一样
				//只测试了H.264
				int mv_stride=0;
...
void Dfanalysis::DrawSample(){
	...
	//CDC和HDC关系
	//CDC是MFC的DC的一个类 
	//HDC是DC的具体，API中的一个句柄类型指针。
	//CDC是设备上下文句柄,相当于一个类的成员变量，m_nHdc是HDC类型的成员变量 
...
{ 
	...
	//位图每个像素所占字节数 
	WORD wBitCount; 
	//根据颜色板大小，位图数据字节大小，位图文件大小，及写入文件字节数 
