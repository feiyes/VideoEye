...
void InputList::OnInputlistExport()
{
	...
	CFileDialog dlg(FALSE,_T("xspf"),_T("InputList.xspf"),NULL,lpszfilter);///TRUE为OPEN对话框FALSE为SAVE AS对话框 
	if(dlg.DoModal()==IDOK) 
...
void InputList::OnM3uImport()
{
	...
	CFileDialog dlg(TRUE,_T("m3u"),NULL,NULL,lpszfilter);//TRUE为OPEN对话框FALSE为SAVE AS对话框 
	if(dlg.DoModal()==IDOK){
		FilePathName=dlg.GetPathName();
...
void InputList::OnM3uExport()
{
	...
	CFileDialog dlg(FALSE,_T("m3u"),_T("InputList.m3u"),NULL,lpszfilter);///TRUE为OPEN对话框FALSE为SAVE AS对话框 
	if(dlg.DoModal()==IDOK) 
		FilePathName=dlg.GetPathName();
