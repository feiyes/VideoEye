...
CAboutDlg::CAboutDlg() : CDialogEx(CAboutDlg::IDD)
{
	...
	bSuccess = VerQueryValue(lpData, 
				  ...
				  &uLen); 
					  //0804中文
					  //04b0为1252,ANSI
					  //可以在ResourceView中的Version的BlockHeader中查看
					  //可以不用关心
	 ...
	 delete [] lpData;//这里不需要释放lpBuffer指向的空间，因为lpBuffer和lpData指向同一块内存空间
#endif
}
...
BOOL CVideoEyeDlg::OnInitDialog()
{
	...
	///////////////显示Splash欢迎界面///////////////////////////////////   
	Welcome welcome;                 //创建欢迎对话框实例  
	welcome.Create(IDB_WELCOME);   
	welcome.CenterWindow();   
	welcome.UpdateWindow();          //send WM_PAINT  
	//显示时间
	Sleep(2000);
	welcome.DestroyWindow();//销毁欢迎窗口   
	/////////////////////////////////////////////////////////////////////////
	//设置背景色
	//SetBackgroundColor(RGB(255,255,255));
	//内存检测
	//在怀疑有内存泄漏的地方(就是当前的地方,只要把断点设在当前行)使用下面
	//的宏会在内存地址为{}处中断
...
// 绘制主图标。对于使用文档/视图模型的 MFC 应用程序，
// 这将由框架自动完成。

void CVideoEyeDlg::OnPaint()
{
	//IsIconic的意思是：是否是最小化状态
	if (IsIconic())
	{
...
void CVideoEyeDlg::OnBnClickedOk()
{
	...
	pThreadPlay = AfxBeginThread(Thread_Play,this);//开始播放线程
	ActiveBtn();
	is_playing=1;
...
int CVideoEyeDlg::GetURL()
{
	...
	strcpy(this->url,W2A(urlstr));
#else
	strcpy(url,urlstr);
	strcpy(this->url,urlstr);
#endif
	if(strcmp(url,"")==0){
		CString resloader;
...
void CVideoEyeDlg::OnBnClickedInputurlbutton()
{
	...
	CFileDialog dlg(TRUE,NULL,NULL,NULL,lpszfilter);///TRUE为OPEN对话框FALSE为SAVE AS对话框 
	if(dlg.DoModal()==IDOK) {
...
void CVideoEyeDlg::OnDropFiles(HDROP hDropInfo)
{
	...
	::DragQueryFile(hDropInfo, 0, pFilePathName,MAX_URL_LENGTH);  // 获取拖放文件的完整路径到输入控件中
	m_inputurl.SetWindowText(pFilePathName);
	::DragFinish(hDropInfo);   // 注意必须调用，用于释放Windows为存放文件拖放信息分配的内存
	free(pFilePathName);
}
...
void CVideoEyeDlg::OnHelpDoc()
{
	...
	//获取exe文件上级路径
	strrchr(realpath, '\\')[0]= '\0';//查找并替换对应字符'\\'为'\0'
...
void CVideoEyeDlg::InitTooltip(){
	...
	CWnd* pW=GetDlgItem(IDOK);//得到按钮指针  
